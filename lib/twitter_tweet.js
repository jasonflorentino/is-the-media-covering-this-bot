...
async function handleNewMention(tweet) {
  ...
  console.log(`${timeId} Received new tweet!`, tweet)

  const tweetId = config.REPLY_IN_CONVERSATION ? tweet.data.conversation_id : tweet.data.id;
  const searchQuery = removeMentionsFromtText(tweet.data.text, tweet.includes.users);
...
function removeMentionsFromtText(text, users) {
  console.log(users);
